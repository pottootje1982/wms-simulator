...
import {
  WorldConfig, Vec3, Shelf, ShelfSlot,
  Elevator, Conveyor, ConveyorCell,
  Robot, Parcel, Operator, Wall,
  TransferTask, FullStatePayload
} from '../types';
...
export function registerRoutes(
  ...
  // ── World ────────────────────────────────────────────────
  app.post('/api/world', async (req: FastifyRequest<{ Body: WorldConfig }>, reply) => {
    const { width, depth, floors } = req.body;
    if (!width || !depth || !floors) return reply.status(400).send({ error: 'width/depth/floors required' });
  ...
  app.get('/api/world', async () => world.toFullState(engine.tick, engine.running, engine.ticksPerSecond));
  ...
  // ── Simulation control ───────────────────────────────────
  app.post('/api/sim/start', async (req: FastifyRequest<{ Body: { ticksPerSecond?: number } }>) => {
    engine.start(req.body?.ticksPerSecond ?? 5);
    return { ok: true, ticksPerSecond: engine.ticksPerSecond };
  });

  app.post('/api/sim/pause', async () => { engine.pause(); return { ok: true }; });
  ...
  app.get('/api/sim/state', async () => world.toFullState(engine.tick, engine.running, engine.ticksPerSecond));

  // ── Shelves ──────────────────────────────────────────────
  app.post('/api/shelves', async (req: FastifyRequest<{
    Body: { label?: string; x: number; y: number; floor: number; rows: number; cols: number }
  }>, reply) => {
  ...
  app.delete('/api/shelves/:id', async (req: FastifyRequest<{ Params: { id: string } }>, reply) => {
  ...
  // ── Walls ────────────────────────────────────────────────
  app.post('/api/walls', async (req: FastifyRequest<{
  ...
  // ── Elevators ────────────────────────────────────────────
  app.post('/api/elevators', async (req: FastifyRequest<{
    Body: { x: number; y: number; floors: number[] }
  }>) => {
    const { x, y, floors } = req.body;
    const id = nanoid();
    const elev: Elevator = {
      id, x, y, floors, currentFloor: floors[0] ?? 0,
  ...
  // ── Conveyors ────────────────────────────────────────────
  app.post('/api/conveyors', async (req: FastifyRequest<{
  ...
  // ── Robots ───────────────────────────────────────────────
  app.post('/api/robots', async (req: FastifyRequest<{
    Body: { name?: string; x: number; y: number; floor: number; color?: string }
  }>) => {
  ...
  app.delete('/api/robots/:id', async (req: FastifyRequest<{ Params: { id: string } }>, reply) => {
    if (!world.robots.has(req.params.id)) return reply.status(404).send({ error: 'Not found' });
  ...
  // ── Operators ────────────────────────────────────────────
  app.post('/api/operators', async (req: FastifyRequest<{
  ...
  // ── Parcels ──────────────────────────────────────────────
  app.post('/api/parcels', async (req: FastifyRequest<{
    Body: {
      label?: string; color?: string;
      shelfId: string; slotRow: number; slotCol: number;
    }
  }>, reply) => {
    ...
    if (slot.parcelId) return reply.status(409).send({ error: 'Slot occupied' });

    const id = nanoid();
    const parcel: Parcel = {
      id, label: label ?? `PKG-${id.slice(0, 4)}`, color,
      status: 'on_shelf', shelfId, slotRow, slotCol
    };
    slot.parcelId = id;
  ...
  // ── Commands ─────────────────────────────────────────────
  app.post('/api/commands/transfer', async (req: FastifyRequest<{
    Body: {
      parcelId: string;
      targetX: number; targetY: number; targetFloor: number;
      targetShelfId?: string; targetSlotRow?: number; targetSlotCol?: number;
      robotId?: string;
    }
  }>, reply) => {
    const { parcelId, targetX, targetY, targetFloor, targetShelfId, targetSlotRow, targetSlotCol, robotId } = req.body;
    if (!world.parcels.has(parcelId)) return reply.status(404).send({ error: 'Parcel not found' });

    const id = nanoid();
    const task: TransferTask = {
      id, parcelId,
      targetPosition: { x: targetX, y: targetY, floor: targetFloor },
      targetShelfId, targetSlotRow, targetSlotCol,
      robotId, status: 'queued', createdAt: engine.tick
    };
